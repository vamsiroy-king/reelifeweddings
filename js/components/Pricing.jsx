      ...
      <div className="relative z-50 mx-auto flex w-fit rounded-full bg-neutral-50 border border-gray-200 p-1">
        ...
        >
          {selected === "0" && (
            <motion.span
              layoutId={"switch"}
              className="absolute top-0 left-0 sm:h-12 h-10 w-full rounded-full border-4 shadow-sm shadow-rose-600 border-rose-600 bg-gradient-to-t from-rose-500 via-rose-400 to-rose-600"
        ...
        >
          ...
          <span className="relative flex items-center gap-2">
            2+ Events
            <span className="rounded-full bg-rose-50 px-2 py-0.5 text-xs font-medium text-black">
              Save 20%
            </span>
          </span>
        </button>
...
export default function PricingSection() {
  ...
  return (
    <div className="w-full px-4 pt-20 pb-20 min-h-screen mx-auto relative bg-[#fdfafaf5]" ref={pricingRef} id="pricing">
      <div
      ...
      <div className="text-center mb-6 max-w-3xl mx-auto relative z-10">
        ...
        >
          ...
          <TimelineContent
            ...
            timelineRef={pricingRef}
            customVariants={revealVariants}
            className="border border-dashed border-rose-500 text-rose-600 px-2 py-1 rounded-xl bg-rose-50 capitalize inline-block"
          >
            wedding
          </TimelineContent>
        </TimelineContent>
      ...
      <div className="w-full flex justify-center py-8 relative z-10">
        <div className="grid md:grid-cols-3 max-w-7xl w-full gap-6 px-4">
          {plans.map((plan, index) => (
            ...
            >
              <Card
                className={`relative w-full h-full flex flex-col border-neutral-200 ${
                  plan.popular ? "ring-2 ring-rose-500 bg-rose-50" : "bg-white shadow-sm"
                }`}
              >
                <CardHeader className="text-left w-full">
                  <div className="flex justify-between items-center w-full">
                    ...
                    {plan.popular && (
                      <div>
                        <span className="bg-rose-500 text-white px-3 py-1 rounded-full text-sm font-medium uppercase tracking-wide">
                          Popular
                        </span>
                      </div>
                    )}
                  ...
                  <div className="flex flex-col">
                     <span className="text-gray-400 text-xl font-medium line-through">
                        ₹{plan.strikeoutPrice.toLocaleString('en-IN')}
                     </span>
                ...
                <CardContent className="pt-0 flex flex-col flex-1">
                  <button
                    ...
                    className={`w-full mb-6 p-4 text-xl font-semibold rounded-xl transition-all ${
                      plan.popular
                        ? "bg-gradient-to-t from-rose-500 to-rose-600 shadow-lg shadow-rose-500/30 border border-rose-400 text-white hover:scale-[1.02]"
                        : "bg-gradient-to-t from-neutral-900 to-neutral-700 shadow-lg shadow-neutral-900/30 border border-neutral-800 text-white hover:scale-[1.02]"
                    }`}
                  >
                    {plan.buttonText}
                  </button>
                  <ul className="space-y-3 font-medium py-3 flex-1">
                  ...
                  <div className="space-y-3 pt-5 border-t border-neutral-200 mt-auto">
                    ...
                    <ul className="space-y-2 font-medium">
                      {plan.includes.slice(1).map((feature, featureIndex) => (
                        <li key={featureIndex} className="flex items-center">
                          <span className="h-5 w-5 bg-green-50 border border-green-500 rounded-full flex items-center justify-center mr-3 flex-shrink-0">
